...
def swig_compile_python_wrapper(out_file, wrapper_source_file, opt)
  ...
  sh "gcc -c -fPIC #{wrapper_source_file} -I/usr/include/python3.4 -I/usr/include/tcl -Isrc -Isrc/extern/platform/src -ltcl -lm -lSDL2 -lSDL2main -o #{out_file}"
end
...
def swig_link_wrapper_module(out_file, wrapper_obj_file, lib_obj_file, opt)
  ...
  sh "ld -shared #{wrapper_obj_file} #{lib_obj_file} -ltcl -lm -lSDL2 -lSDL2main -o #{our_file}"
end
...
def swig_create_library(language, opt)
  ...
  swig_interface_file          = File.join(out_dir, 'swig.h')
  ...
  library_obj_file             = File.join($opt['build_dir'] + 'ort.so')
  final_shared_library         = File.join(out_dir, '_ort.so')
